var SkyRTC = function() {
    ...
    var iceServer = {
        "iceServers": [
        {
            "url": 'stun:api.shishimao.com:3478'//"stun:stun.services.mozilla.com"//stun:stun.l.google.com:19302   stun:api.shishimao.com:3478"
        }]
    };
    iceServer={
        "iceServers":
        [{
            ...
            "username":"learningtech",
            "credential":"learningtech"
        }]
    }
    var packetSize = 1000;
    ...
    room为房间号字符串
    */
    skyrtc.prototype.connect = function(server, room) {//skyrtc类最先被调用的方法
        var socket,
            that = this;
        room = room || "";
        ...
        socket.onmessage = function(message) {
            ...
            if(json.__relay){//转发给房间内所有人
                delete json.__relay;
                that.broadcastBase(json);
            }
            if (json.eventName) {
        ...
        this.on("_ice_candidate", function(data) {//收到服务器发来的对端网络地址描述
            ...
            console.log('对端的candidate：'+data.candidate);
        });
    ...
    //创建本地流
    skyrtc.prototype.createStream = function(options) {//与服务器建立socket连接之后
        var that = this;
    ...
    //广播信息
    skyrtc.prototype.broadcastBase=function(data){
        var socketId;
        for (socketId in this.dataChannels) {
    ...
    //发送的基本方法
    skyrtc.prototype.send=function(data,socketId){
    ...
    skyrtc.prototype.addDataChannel = function(socketId, channel) {
        ...
        channel.onmessage = function(message) {
            ...
            }else if(json.type==='__relay'&&that.socket&&that.socket.readyState==1){
                that.socket.send(JSON.stringify(json.data));
            } else {
